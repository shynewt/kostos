...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const matchingCurrency = CURRENCY_OPTIONS.find(c => c.symbol === currencyCode || c.code === currencyCode);
    if (matchingCurrency) {
    ...
    // --- Create Members --- 
    await Promise.all(importData.participants.map(async (participant) => {
    ...
    // --- Create Categories --- 
    await Promise.all(importData.categories.map(async (category) => {
    ...
    await Promise.all(importData.paymentMethods.map(async (method) => {
    ...
    await Promise.all(importData.expenses.map(async (expense) => {
      ...
      const newPaymentMethodId = originalPaymentMethodId ? paymentMethodIdMapping[originalPaymentMethodId] : null;
      ...
      // Create Splits
      await Promise.all(expense.paidFor.map(async (split) => {
        ...
        if (!newSplitMemberId) {
            console.warn(`Skipping split for expense '${expense.title}' - could not find mapping for original member ID ${originalSplitMemberId}`);
