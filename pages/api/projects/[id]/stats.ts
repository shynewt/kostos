...
async function getProjectStats(req: NextApiRequest, res: NextApiResponse, projectId: string) {
  try {
    ...
    const expensesByCategory = categories.map((category) => {
      ...
      const categoryExpensesWithDetails = expensesWithDetails.filter(
    ...
    const expensesByPaymentMethod = paymentMethods.map((method) => {
      ...
      const methodExpensesWithDetails = expensesWithDetails.filter(
        (expense) => expense.paymentMethodId === method.id
      )
    ...
    const noPaymentMethodAverage =
      noPaymentMethodExpenses.length > 0 ? noPaymentMethodTotal / noPaymentMethodExpenses.length : 0
    const noPaymentMethodLargestExpense =
      noPaymentMethodExpenses.length > 0
        ? noPaymentMethodExpenses.reduce(
            (max, expense) => (expense.amount > max.amount ? expense : max),
            noPaymentMethodExpenses[0]
          )
        : null

    // Calculate member spending for expenses with no payment method
    const noPaymentMethodExpensesWithDetails = expensesWithDetails.filter(
      (expense) => !expense.paymentMethodId
    )
    ...
    const memberSpending = members.map((member) => {
      ...
      const memberCategories = categories
        .map((category) => {
          const categoryExpenses = expensesWithDetails.filter((expense) => expense.categoryId === category.id)
      ...
      const memberPaymentMethods = paymentMethods
        .map((method) => {
          const methodExpenses = expensesWithDetails.filter(
            (expense) => expense.paymentMethodId === method.id
          )
      ...
      const memberExpenseCount = expensesWithDetails.filter(
        (expense) =>
    ...
    const monthlyTrendData =
      sortedExpensesByMonth.length >= 2
        ? sortedExpensesByMonth.map((current, index, array) => {
            if (index === 0) return { ...current, changePercent: 0 }

            const previous = array[index - 1]
            const changePercent =
              previous.amount === 0 ? 100 : ((current.amount - previous.amount) / previous.amount) * 100
